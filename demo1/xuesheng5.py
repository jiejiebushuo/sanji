...
class StudentManagementApp:
    ...
    def open_form(self, title, save_command, record=None):
        ...
        for i, (field, info) in enumerate(self.fields.items()):
            tk.Label(form_window, text=info["label"]).grid(row=i, column=0, pady=5, padx=10)
            ...
            if info["type"] == "entry":
                ...
                for j, option in enumerate(info["options"]):
                    tk.Radiobutton(form_window, text=option, variable=var, value=option).grid(row=i, column=1 + j, pady=5, padx=10)
        ...
        save_button.grid(row=len(self.fields), columnspan=2, pady=10)
